...
username="bearychat" # Default username to post messages.
...
if [[ -e "/etc/bearychat.conf" ]]; then
    . /etc/bearychat.conf
fi

if [[ -n "$HOME" && -e "$HOME/.bearychat" ]]; then
    . "$HOME/.bearychat"
fi

# Overwrite webhook_url if the environment variable BEARYCHAT_WEBHOOK is set
if [[ "$BEARYCHAT_WEBHOOK" != "" ]]; then
    webhook_url=$BEARYCHAT_WEBHOOK
fi

# Overwrite upload_token if the environment variable BEARYCHAT_TOKEN is set
if [[ "$BEARYCHAT_TOKEN" != "" ]]; then
    upload_token=$BEARYCHAT_TOKEN
fi
