...
*bearychat.sh* is a bash script that works like [tee](http://en.wikipedia.org/wiki/Tee_(command)) command.
Instead of writing the standard input to files, *bearychat.sh* posts it to [BearyChat](https://bearychat.com/).
...
*bearychat.sh* uses [curl](http://curl.haxx.se/) command to communicate with Slack.
...
```
webhook_url=""      # Incoming Webhooks integration URL. See https://github.com/bearyinnovative/bearychat-tutorial/blob/master/robots/incoming.md
...
usage: slacktee.sh [options]
  options:
    ...
    --setup                           Set up .bearychat file interactively.
```
